...
# 速度限制默认值（m/s, rad/s）| Default velocity limits
MAX_HORIZONTAL_SPEED = 5.0   # 最大水平合速度 (m/s) | Max horizontal speed magnitude
MAX_VERTICAL_SPEED = 3.0     # 最大垂直速度绝对值 (m/s) | Max vertical speed (absolute)
MAX_YAW_RATE = 1.0           # 最大偏航角速度绝对值 (rad/s) | Max yaw rate (absolute)


# ---------------- 参数解析 | Parameter Parsing ----------------
def _parse_px4_parameters_hpp(hpp_path: str = None) -> dict:
    """
    解析 PX4 编译生成的 px4_parameters.hpp，提取参数名 -> param_t 索引映射
    Parse PX4 generated px4_parameters.hpp to extract parameter name -> param_t index mapping
    
    参数 | Args:
    - hpp_path: px4_parameters.hpp 的绝对路径，默认自动搜索 | Absolute path to px4_parameters.hpp, auto-detected by default
    
    返回 | Returns:
    - dict: {参数名: param_t 索引} | {param_name: param_t index}
    """
    if hpp_path is None:
        # 自动搜索常见路径 | Auto-detect common paths
        candidates = [
            os.path.expanduser("~/PX4-Autopilot/build/px4_sitl_default/src/lib/parameters/px4_parameters.hpp"),
        ]
        for p in candidates:
            if os.path.isfile(p):
                hpp_path = p
    ...
    if hpp_path is None or not os.path.isfile(hpp_path):
        return {}
    ...
    try:
        ...
        # 定位 enum class params : uint16_t { ... };
        enum_start = content.find('enum class params : uint16_t {')
        if enum_start == -1:
            return {}
        ...
        # 提取所有枚举值（按出现顺序，索引从 0 开始）
        # 匹配形如 "MPC_XY_VEL_MAX," 的行
        values = re.findall(r'^\s*([A-Z][A-Z_0-9]*)\s*,', enum_section, re.MULTILINE)
        for idx, name in enumerate(values):
            param_map[name] = idx

    except Exception as e:
        print(f"⚠️ Failed to parse px4_parameters.hpp: {e}")
...
class OffboardControl(Node):
    ...
    def __init__(self):
        ...
        # 参数修改发布者 | Parameter set request publisher
        self.parameter_set_request_publisher = self.create_publisher(
            ParameterSetValueRequest, f'{namespace}/fmu/in/parameter_primary_set_value_request', qos_profile)
        ...
        # 参数修改响应订阅者 | Parameter set response subscriber
        self.parameter_set_response_subscriber = self.create_subscription(
            ParameterSetValueResponse,
            f'{namespace}/fmu/out/parameter_primary_set_value_response',
            self.parameter_set_response_callback,
            qos_profile
        )

        self.get_logger().info("[SUB] Subscribed to local_position/status/attitude/lidar_scan/parameter_response")
        ...
        # 参数映射与请求跟踪 | Parameter map & request tracking
        self._param_map = _parse_px4_parameters_hpp()
        if self._param_map:
            self.get_logger().info(f"📋 [PARAM] Loaded {len(self._param_map)} parameter mappings from px4_parameters.hpp")
        else:
            self.get_logger().warning("⚠️ [PARAM] Failed to load parameter mappings; set_px4_param() will not work")
        self._pending_param_requests = {}  # {timestamp: threading.Event}
        self._param_response_results = {}  # {timestamp: bool}
        self._param_lock = threading.Lock()

        # MAVLink 参数修改客户端（用于 SITL，因为 DDS parameter_primary 不可用）
        self._mavlink_conn = None
        self._mavlink_target_system = 1
        self._mavlink_target_component = 1
    ...
    def parameter_set_response_callback(self, msg: ParameterSetValueResponse):
        """
        参数修改响应回调：匹配 pending request 并通知等待线程
        Parameter set response callback: match pending request and notify waiting thread
        
        Note: Due to time base conversion in DDS client (ROS2 sys_time → PX4 hrt),
        request_timestamp may not match exactly. We use parameter_index as fallback.
        """
        req_ts = msg.request_timestamp
        with self._param_lock:
            # Try exact timestamp match first
            if req_ts in self._pending_param_requests:
                self._param_response_results[req_ts] = True
                self._pending_param_requests[req_ts].set()
                self.get_logger().debug(f"[PARAM] Response received for timestamp={req_ts}, param_index={msg.parameter_index}")
                return
            
            # Fallback: trigger the first pending event (parameter sets are sequential)
            for ts, event in list(self._pending_param_requests.items()):
                if isinstance(event, threading.Event):
                    self._param_response_results[ts] = True
                    event.set()
                    self.get_logger().debug(f"[PARAM] Response fallback-matched for param_index={msg.parameter_index}")
                    return


    def set_px4_param(self, param_name: str, value, timeout: float = 2.0) -> bool:
        """
        同步修改 PX4 飞控参数（通过 DDS Topic Request/Response）
        Synchronously modify a PX4 parameter via DDS Topic Request/Response
        
        参数 | Args:
        - param_name: 参数名称字符串，如 "MPC_XY_VEL_MAX" | Parameter name string, e.g. "MPC_XY_VEL_MAX"
        - value: 参数值（int 或 float）| Parameter value (int or float)
        - timeout: 等待响应的超时时间（秒）| Timeout for waiting response (seconds)
        
        返回 | Returns:
        - bool: 是否成功修改 | True if parameter was set successfully
        """
        if not self._param_map:
            self.get_logger().error("❌ [PARAM] Parameter map not loaded. Cannot set parameter.")
            return False

        if param_name not in self._param_map:
            self.get_logger().error(f"❌ [PARAM] Unknown parameter: {param_name}")
            return False

        param_index = self._param_map[param_name]
        timestamp = int(self.get_clock().now().nanoseconds / 1000)

        req = ParameterSetValueRequest()
        req.timestamp = timestamp
        req.parameter_index = param_index
        ...
        if isinstance(value, int):
            ...
            req.int_value = 0
            req.float_value = value
        else:
            self.get_logger().error(f"❌ [PARAM] Value must be int or float, got {type(value)}")
            return False

        # 创建等待事件 | Create wait event
        event = threading.Event()
        with self._param_lock:
            self._pending_param_requests[timestamp] = event
            self._param_response_results[timestamp] = False

        self.get_logger().info(f"🔧 [PARAM] Setting {param_name} (index={param_index}) = {value}")
        self.parameter_set_request_publisher.publish(req)

        # 阻塞等待响应 | Block waiting for response
        success = event.wait(timeout=timeout)

        with self._param_lock:
            result = self._param_response_results.get(timestamp, False)
            del self._pending_param_requests[timestamp]
            del self._param_response_results[timestamp]

        if success and result:
            self.get_logger().info(f"✅ [PARAM] {param_name} set to {value} successfully")
            return True
        else:
            self.get_logger().warning(f"⚠️ [PARAM] {param_name} set timeout or failed")
            return False


    def _ensure_mavlink_connection(self) -> bool:
        """
        确保 MAVLink 连接已建立（用于 SITL 参数修改）
        Ensure MAVLink connection is established (for SITL parameter modification)
        """
        if self._mavlink_conn is not None:
            return True
        try:
            self.get_logger().info("🔗 [MAVLINK] Connecting to PX4 SITL (udpin:127.0.0.1:14550)...")
            self._mavlink_conn = mavutil.mavlink_connection('udpin:127.0.0.1:14550', source_system=255)
            # Wait for heartbeat to identify target system/component
            start = time.time()
            while time.time() - start < 5:
                msg = self._mavlink_conn.recv_match(type='HEARTBEAT', blocking=False)
                if msg:
                    self._mavlink_target_system = self._mavlink_conn.target_system
                    self._mavlink_target_component = self._mavlink_conn.target_component
                    self.get_logger().info(
                        f"🔗 [MAVLINK] Connected to system {self._mavlink_target_system}, "
                        f"component {self._mavlink_target_component}"
                    )
                    return True
                time.sleep(0.05)
            self.get_logger().warning("⚠️ [MAVLINK] No heartbeat received, using default system=1, component=1")
            return True  # Still usable with defaults
        except Exception as e:
            self.get_logger().error(f"❌ [MAVLINK] Connection failed: {e}")
            return False

    def set_px4_param_mavlink(self, param_name: str, value, timeout: float = 5.0) -> bool:
        """
        通过 MAVLink 修改 PX4 参数（适用于 SITL，绕过 DDS parameter_primary 限制）
        Modify PX4 parameter via MAVLink (for SITL, bypassing DDS parameter_primary limitation)

        参数 | Args:
        - param_name: 参数名称字符串，如 "MPC_XY_VEL_MAX"
        - value: 参数值（int 或 float）
        - timeout: 等待确认的超时时间（秒）

        返回 | Returns:
        - bool: 是否成功修改
        """
        if not self._ensure_mavlink_connection():
            return False

        try:
            self.get_logger().info(f"🔧 [MAVLINK PARAM] Setting {param_name} = {value}")

            # 1. Clear any stale PARAM_VALUE messages from buffer
            for _ in range(50):
                old = self._mavlink_conn.recv_match(type='PARAM_VALUE', blocking=False)
                if old is None:
                    break

            # 2. Send PARAM_SET
            self._mavlink_conn.mav.param_set_send(
                self._mavlink_target_system,
                self._mavlink_target_component,
                param_name.encode(),
                float(value),
                mavutil.mavlink.MAV_PARAM_TYPE_REAL32
            )

            # 3. Also send a PARAM_REQUEST_READ to force a response
            time.sleep(0.1)
            self._mavlink_conn.mav.param_request_read_send(
                self._mavlink_target_system,
                self._mavlink_target_component,
                param_name.encode(),
                -1
            ...
            # 4. Wait for PARAM_VALUE confirmation (ignore stale/old values)
            start = time.time()
            while time.time() - start < timeout:
                msg = self._mavlink_conn.recv_match(type='PARAM_VALUE', blocking=False)
                if msg:
                    pname = msg.param_id.strip()
                    if pname == param_name:
                        if abs(msg.param_value - float(value)) < 0.01:
                            self.get_logger().info(
                                f"✅ [MAVLINK PARAM] {param_name} set to {value} successfully"
                            )
                            return True
                        # If we get a mismatch, it might be a stale broadcast;
                        # keep waiting for the updated value
                time.sleep(0.05)

            self.get_logger().warning(f"⚠️ [MAVLINK PARAM] {param_name} set timeout")
            return False

        except Exception as e:
            self.get_logger().error(f"❌ [MAVLINK PARAM] Error setting {param_name}: {e}")
            return False

    def set_speed_limits_by_param_mavlink(self, max_horizontal: float = None, max_vertical_up: float = None,
                                          max_vertical_down: float = None, max_yaw_rate: float = None,
                                          timeout: float = 5.0) -> dict:
        """
        通过 MAVLink 修改飞控参数来设置速度限制（SITL 推荐方式）
        Set speed limits via MAVLink parameter modification (recommended for SITL)
        """
        results = {}
        param_mapping = {
            'MPC_XY_VEL_MAX': max_horizontal,
            'MPC_Z_VEL_MAX_UP': max_vertical_up,
            'MPC_Z_VEL_MAX_DN': max_vertical_down,
            'MC_YAWRATE_MAX': max_yaw_rate,
        }
        for param_name, val in param_mapping.items():
            if val is not None:
                results[param_name] = self.set_px4_param_mavlink(param_name, float(val), timeout=timeout)
        return results

    def set_speed_limits_by_param(self, max_horizontal: float = None, max_vertical_up: float = None,
                                  max_vertical_down: float = None, max_yaw_rate: float = None,
                                  timeout: float = 2.0) -> dict:
        """
        通过修改飞控参数来设置速度限制（全局生效，自动持久化）
        Set speed limits by modifying PX4 parameters (global effect, auto-saved)
        
        参数 | Args:
        - max_horizontal: 最大水平速度 (m/s)，对应 MPC_XY_VEL_MAX | Max horizontal speed, maps to MPC_XY_VEL_MAX
        - max_vertical_up: 最大上升速度 (m/s)，对应 MPC_Z_VEL_MAX_UP | Max upward speed, maps to MPC_Z_VEL_MAX_UP
        - max_vertical_down: 最大下降速度 (m/s)，对应 MPC_Z_VEL_MAX_DN | Max downward speed, maps to MPC_Z_VEL_MAX_DN
        - max_yaw_rate: 最大偏航速率 (rad/s)，对应 MPC_YAWRATE_MAX | Max yaw rate, maps to MPC_YAWRATE_MAX
        - timeout: 每条参数修改的超时时间（秒）| Timeout per parameter modification
        
        返回 | Returns:
        - dict: {参数名: 是否成功} | {param_name: success_bool}
        """
        results = {}
        param_mapping = {
            'MPC_XY_VEL_MAX': max_horizontal,
        ...
        for param_name, value in param_mapping.items():
            if value is not None:
                results[param_name] = self.set_px4_param(param_name, float(value), timeout=timeout)
    ...
    def is_obstacle_ahead(self, safe_dist: float, front_angle) -> bool:
    ...
    def fly_to_trajectory_setpoint_dwa(self, x, y, z, yaw, timeout=600.0) -> bool:
        ...
        while rclpy.ok() and time.time() - start < timeout:
            ...
            obstacle_ahead = self.is_obstacle_ahead(safe_dist = 1.5, front_angle=60)
            if (not self.dwa_active) and obstacle_ahead:
                self.dwa_active = True
                self.get_logger().info("🚧 [DWA] Entering local avoidance mode")
            elif self.dwa_active and (not self.is_obstacle_ahead(safe_dist * 1.05)):
...
class Vehicle:
    ...
    def set_px4_param(self, param_name: str, value, timeout: float = 2.0) -> bool:
        """
        代理：同步修改 PX4 飞控参数（DDS Topic 方式）
        Proxy: synchronously modify a PX4 parameter (DDS Topic method)
        """
        return self.drone.set_px4_param(param_name, value, timeout)

    def set_speed_limits_by_param(self, max_horizontal: float = None, max_vertical_up: float = None,
                                  max_vertical_down: float = None, max_yaw_rate: float = None,
                                  timeout: float = 2.0) -> dict:
        """
        代理：通过修改飞控参数设置速度限制（DDS Topic 方式）
        Proxy: set speed limits by modifying PX4 parameters (DDS Topic method)
        """
        return self.drone.set_speed_limits_by_param(max_horizontal, max_vertical_up, max_vertical_down, max_yaw_rate, timeout)

    def set_px4_param_mavlink(self, param_name: str, value, timeout: float = 5.0) -> bool:
        """
        代理：通过 MAVLink 修改 PX4 参数（SITL 推荐方式）
        Proxy: modify PX4 parameter via MAVLink (recommended for SITL)
        """
        return self.drone.set_px4_param_mavlink(param_name, value, timeout)

    def set_speed_limits_by_param_mavlink(self, max_horizontal: float = None, max_vertical_up: float = None,
                                          max_vertical_down: float = None, max_yaw_rate: float = None,
                                          timeout: float = 5.0) -> dict:
        """
        代理：通过 MAVLink 修改飞控参数设置速度限制（SITL 推荐方式）
        Proxy: set speed limits via MAVLink parameter modification (recommended for SITL)
        """
        return self.drone.set_speed_limits_by_param_mavlink(max_horizontal, max_vertical_up, max_vertical_down, max_yaw_rate, timeout)
